...
| `Matrix_M` | PE 阵列行数 | 4 |
| `Matrix_N` | PE 阵列列数 | 4 |
| `ReduceWidth` | 每个 PE 的归约位宽 (bit) | `ReduceWidthByte × 8`（默认 512） |
| `ScaleWidth` | 缩放因子位宽 | 依数据类型计算 |
| `ResultWidth` | 输出结果位宽 (bit) | 32 |
| PE 流水级数 | FReducePE 内部 | 5 级 |
...
例：4×4 阵列、512-bit 归约宽度、INT8、2GHz：
`2GHz × 4 × 4 × (512/8) × 2 = 4 TOPS`

## 3. 功能描述

MatrixTE 是 CUTE 的核心计算模块，执行矩阵乘法的乘加运算。它接收来自 DataController 的 A/B 矩阵数据和 C 矩阵累加值，计算 `D = A × B + C`。
...
- 块缩放矩阵乘法：支持 MXFP8/MXFP4/NVFP4 等需要逐块缩放的格式
...
- **广播机制**：同一行的 PE 接收相同的 A 向量数据，同一列的 PE 接收相同的 B 向量数据
- **Scale 广播**：ScaleA 按 PE 行广播，ScaleB 按 PE 列广播

### 4.2 FReducePE 5 级流水线

```
Pipe0         Pipe1          Pipe2          Pipe3         Pipe4
Decode  →  Exponent Cmp  →  Align+Reduce → Accumulate → Normalize
```
...
| **Pipe2 (Align + Reduce)** | 根据指数差对齐尾数；部分和归约树；FP4 缩放对齐 |
| **Pipe3 (Accumulate)** | 最终累加；加入 C 矩阵的尾数值 |
| **Pipe4 (Normalize)** | CLZ 前导零归一化；指数调整；异常处理；打包为 FP32 结果 |
...
| MXFP8 / MXFP4 / NVFP4 | 先解码为内部表示，再乘以 Scale 因子 |
| FP4 | 扩展为内部表示 |
