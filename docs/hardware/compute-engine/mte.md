...
每个 PE 内部的 FPE 采用四阶段逻辑架构，寄存器可在阶段之间或阶段内部灵活插入：

```
阶段 A           阶段 B            阶段 C          阶段 D
┌──────────┐   ┌──────────────┐   ┌──────────┐   ┌──────────┐
│ 预处理    │ → │ 全局对阶与对齐│ → │ 归约与累加│ → │ 后处理    │
│ 与解码    │   │              │   │          │   │ 与规格化  │
│ ·解码     │   │ ·CmpTree     │   │ ·符号注入│   │ ·CLZ     │
│ ·双路径   │   │ ·偏移计算    │   │ ·加法树  │   │ ·舍入    │
│ ·尾数乘法 │   │ ·27-bit对齐  │   │ ·溢出保护│   │ ·异常处理│
│ ·指数预算 │   │              │   │ ·C值合并 │   │ ·结果打包│
└──────────┘   └──────────────┘   └──────────┘   └──────────┘
     ↕ 可插拔寄存器    ↕ 可插拔寄存器    ↕ 可插拔寄存器
```
...
```
ScaleA ──→ MatrixTE ──→ FPE 阶段 A
                         有效乘积指数 = A.data_exp + A.scale_exp + B.data_exp + B.scale_exp
                                              │
                                              ↓
                         FPE 阶段 B: CmpTree 指数比较（含 Scale 效应的最大值）
```
...
```
                         ┌─── TaskController ── dataType / ConfigInfo
                         │
ADataController ──VectorA──→ MatrixTE (M×N FPE Array)
BDataController ──VectorB──→         │
AScaleController ─ScaleA──→          │
BScaleController ─ScaleB──→          │
CDataController ──MatrixC──→         │
                                 MatrixTE
                         ┌──────MatrixD──→ CDataController
                         │
                   AfterOps (透传)
                         │
                         ↓
