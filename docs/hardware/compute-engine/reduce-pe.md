...
> **注意**：`ReducePE.scala` 已被注释废弃，当前使用 `FReducePE.scala` 作为统一的 PE 实现。
...
VectorA ──┐
           ├─→ ┌────────────────────────────────────────────────────────┐
VectorB ──┘    │  阶段 A          阶段 B           阶段 C          阶段 D  │
               │  预处理与解码  →  全局对阶与对齐  →  归约与累加  →  后处理  │
ScaleA  ─────→ │  · 多格式解码    · CmpTree        · 符号注入      · CLZ   │
ScaleB  ─────→ │  · 双路径并行    · 偏移计算        · 加法树        · 舍入   │
CAdd    ─────→ │  · 尾数乘法      · 27-bit 对齐    · 溢出保护      · 打包   │
opcode  ─────→ │  · 指数预算      ·                 ·               │        │
               └──────────────────────┬─────────────────────────────────┘
                                      │
                                  D[32bit]
...
FPE 的关键参数由 `CuteFPEParams` 定义（源码：`CUTEParameters.scala:350`）：
...
`RawFloat` 内部表示（源码：`FReducePE.scala:13`）：
...
**解码的核心逻辑** `RawFloat.fromUInt`（源码：`FReducePE.scala:21-36`）：
...
**位并行比较策略**（源码：`FReducePE.scala:120-147`）：
...
**精度保证：** 采用 27-bit 的原因是，在 33 路数据累加时，对齐移位的精度损失通过保护位和粘滞位被完整记录。即使在处理大尺寸矩阵运算（数千次累加）时，单次归约的 ULP 误差也能控制在极低水平，有效抑制长程精度漂移。
...
**舍入模式：** 采用 RNE（Round to Nearest, ties to Even）舍入策略。利用阶段 B 中保留的 Guard/Round/Sticky 三位判定舍入方向：
...
采用 27-bit 对齐（23-bit 有效尾数 + 3-bit 保护/舍入/粘滞位），在 33 路数据累加时保持极低的 ULP 误差。这在处理大尺寸矩阵运算（如 LLaMA 等大模型的 GEMM）时能有效抑制数千次累加带来的精度漂移。
...
| E2M1 × E2M1 → F32 | 规划中 | 4 bit | FP32 | FP32 | 1 或 2 |

> **说明**：当前源码（`FReducePE.scala`）实现了 opcode 0~6 的解码和计算逻辑（阶段 A~D 的基础框架）。opcode 7~12（FP8/FP4 等微缩放格式）和 Flow 2（E2M1 高吞吐）为规划中的功能扩展，对应的数据路径在设计中已预留。

## 11. 流水线控制

### 11.1 反压机制

FPE 采用寄存器反压机制控制数据流动（源码：`FReducePE.scala:640-661`）。各级之间通过 `AllowIn` 信号传播反压：当后级阻塞时，前级暂停接受新数据，形成全流水线的反压链。
...
Scale 因子在 MatrixTE 层面传入，其指数在阶段 A 叠加到各元素的乘积指数中，参与 CmpTree 的最大值比较。
