...
### 2.4 A/B Scale Loader

| 参数 | ASL | BSL |
|------|-----|-----|
| 源文件 | `AScaleLoader.scala` | `BScaleLoader.scala` |
| 数据源 | 主存（通过 LocalMMU → TileLink） |
| 数据目标 | A Scale Scratchpad | B Scale Scratchpad |
| 加载量 | `ScaratchpadTensor_K × ScaratchpadTensor_M × ScaleVecWidth(dataType) / outsideDataWidthByte` | `ScaratchpadTensor_K × ScaratchpadTensor_N × ScaleVecWidth(dataType) / outsideDataWidthByte` |
| 适用场景 | 块缩放数据类型（MXFP/NVFP） | 块缩放数据类型（MXFP/NVFP） |

Scale Loader 与数据 Loader 结构类似，使用相同的状态机模式（`idle → mm_task`，含 `s_load_init → s_load_working → s_load_end` 访存状态机），维护 Source ID CAM 映射请求到 Scratchpad 地址。

## 3. 功能描述

Memory Loader 是 CUTE 与主存之间的桥梁，负责将矩阵 tile 数据从 DRAM 搬运到 Scratchpad。Scale Loader 负责搬运对应的缩放因子。

**核心工作流程（数据 Loader 和 Scale Loader 共用）：**
